...
require_once 'Mandrill/Error.php';

class Mandrill extends \AbstractController {
    public $api;
    public $apikey;
    public $ch;
    ...
    public function __construct($apikey=null,$api) {
    ...
    public function castError($result) {
        ...
        $class = "xEnquiryNSubscription\\$class";
        // throw new \Exception($class, 1);
    ...
    public function send($to,$from=null,$subject,$body,$header="",$ccs=array(),$bccs=array()){
        $message = array(
        'html' => $body,
        'text' => $body,
        'subject' => $subject,
        'from_email' => $this->api->current_website['email_from'],
        'from_name' => $this->api->current_website['email_from_name'],
        'to' => array(
            array(
                'email' => $to,
                'type' => 'to'
        ...
        'headers' => array('Reply-To' => $this->api->current_website['email_reply_to']),        
        );

        foreach ($bccs as $bce) {
            $message['to'][] = array('email'=>$bce,'type'=>'bcc');
        }

        $async = true;
        $ip_pool = 'Main Pool';
        // $send_at = date('Y-m-d H:i:s');
        $result = $this->messages->send($message, $async, $ip_pool);
        return $result;
    }
